...
public class InscriptionEvenementServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        String action       = req.getParameter("action"); // "inscrire" ou "desinscrire"

        try {
            if ("desinscrire".equals(action)) {
                DAOFactory.getInscriptionDAO().delete(user.getPseudo(), nomEvenement);
            } else {
                String instrument = req.getParameter("instrument");
                String status     = "présent";
                Inscrit i = new Inscrit(user.getPseudo(), nomEvenement, instrument, status);
                DAOFactory.getInscriptionDAO().upsert(i);
            }
            resp.sendRedirect(req.getContextPath() + "/evenement?nom=" + encodeUrl(nomEvenement));
